...
func (dic DatabricksImageContent) MarshalJSON() ([]byte, error) {
	r := struct {
		Type   string            `json:"type"`
...
func (dic *DatabricksImageContent) UnmarshalJSON(data []byte) error {
	...
	if !ok {
		return fmt.Errorf(`missing "type" field in DatabricksImageContent`)
	}
	dic.Type = typeVal
	source, ok := m["source"].(map[string]any)
	if !ok {
		return fmt.Errorf("invalid source field in DatabricksImageContent")
	}
	base64Data, ok := source["data"].(string)
	if !ok {
		return fmt.Errorf("invalid data field in DatabricksImageContent")
	}
...
func (tc ToolCall) MarshalJSON() ([]byte, error) {
	...
	if tc.Signature != "" {
		inner["signature"] = tc.Signature
	}
	tcMap["tool_call"] = inner
	return json.Marshal(tcMap)
}
...
func (tc *ToolCall) UnmarshalJSON(data []byte) error {
	...
	tc.Type = typ
	tc.FunctionCall = &fc
	if sig, ok := toolCall["signature"].(string); ok {
		tc.Signature = sig
	}
	return nil
}
...
func (tc *ToolCallResponse) UnmarshalJSON(data []byte) error {
	...
	tc.Name = name
	tc.Content = content
	if sig, ok := tr["signature"].(string); ok {
		tc.Signature = sig
	}
	return nil
}
