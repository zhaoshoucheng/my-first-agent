...
const (
	...
	APITypeAzureDataBricks APIType = "AZURE_DATABRICKS"
)
...
func IsAzure(apiType APIType) bool {
	return apiType == APITypeAzure || apiType == APITypeAzureAD || apiType == APITypeAzureDataBricks
}

// GetApiType returns the configured API type for the client.
...
func (c *Client) setHeaders(req *http.Request) {
	...
	if c.apiType == APITypeOpenAI || c.apiType == APITypeAzureAD || c.apiType == APITypeAzureDataBricks {
...
func (c *Client) buildURL(suffix string, model string) string {
	if IsAzure(c.apiType) {
		if c.Model != "" {
			return c.buildAzureURL(suffix, c.Model)
		} else {
...
func (c *Client) buildAzureURL(suffix string, model string) string {
	...
	if c.apiType == APITypeAzureDataBricks {
		// azure databricks
		return fmt.Sprintf("%s/serving-endpoints/%s/invocations", baseURL, model)
	}
	// azure example url:
